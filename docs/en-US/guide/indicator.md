...
<Tip title="Tip" :tip="['Some indicators can be overlaid on candles using <code>chart.createIndicator(\'MA\', true, { id: \'candle_pane\' })</code>, while some cannot. Built-in candle-compatible indicators are: BBI, BOLL, EMA, MA, SAR, SMA.']"/>
