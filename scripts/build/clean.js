...
function clean() {
  ...
  start('Cleaning build output...', [`target: ${buildDir}`])

  if (!fs.existsSync(buildDir)) {
    success('No build output to clean', startTime, ['deleted: 0 files'])
    return
  }
  ...
  success('Cleaned build output', startTime, [`deleted: ${deletedFileCount} files`])
}
