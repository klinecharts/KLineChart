   ...
   _drawArea (candleOptions) {
     ...
     const onDrawEnd = () => {
       ...
       if (linePoints.length > 0) {
         ...
         this._ctx.strokeStyle = isOnline ? areaOptions.lineColor : areaOptions.lineColorOffline
       ...
       if (areaPoints.length > 0) {
         ...
         if (isArray(fillColor)) {
           ...
           try {
             fillColor.forEach(({ offset, color, offlineColor }) => {
               gradient.addColorStop(offset, isOnline ? color : offlineColor)
             })
           } catch (e) {
           }
